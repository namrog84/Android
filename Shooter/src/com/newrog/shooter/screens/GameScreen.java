...
public class GameScreen implements Screen {
	...
	public void render (float delta) {
		...
		for (int i = entities.size() - 1; i >= 0; --i) {
			...
			ArrayList<Entity> collider = collidedC(e);
			if (collider != null) {
				...
				for (int z = 0; z < collider.size(); ++z) {
					...
					if (e instanceof Enemy && collider.get(z) instanceof Bomb) {
						e.life = 0;
					}else 
		...
		for (int i = 0; i < powerUps.size; ++i) {
			...
			if (powerUps.get(i).rect.overlaps(p.rect)) {
				if (powerUps.get(i).type == PowerUp.PType.SPREAD) p.ammo += 200;
				if (powerUps.get(i).type == PowerUp.PType.MISSILE) p.weaponToggle = !p.weaponToggle;
				if (powerUps.get(i).type == PowerUp.PType.BOMB) {
					new Bomb(game, powerUps.get(i).getX(),powerUps.get(i).getY());
...
// camera.translate(10, 0, 0);
	...
	private ArrayList<Entity> collided (Entity entity) {
		...
		for (int i = 0; i < entities.size(); i++) {
			...
			if (entity.rect.overlaps(entities.get(i).rect)) {
				if (collisions == null) collisions = new ArrayList<Entity>();
	...
	private ArrayList<Entity> collidedC (Entity ent) {
		ArrayList<Entity> collisions = null;
		for (int i = 0; i < entities.size(); i++) {
			if (ent == entities.get(i)) continue;
			Entity e = entities.get(i);
			if ((ent.circle.x - e.circle.x) * (ent.circle.x - e.circle.x) + (ent.circle.y - e.circle.y)
				* (ent.circle.y - e.circle.y) < (ent.circle.radius + e.circle.radius) * (ent.circle.radius + e.circle.radius)) {
				if (collisions == null) collisions = new ArrayList<Entity>();
				collisions.add(entities.get(i));
			}
	...
	public class customComparator implements Comparator<Entity> {
		...
		public int compare (Entity e1, Entity e2) {
			if (e1.zIndex < e2.zIndex) return 1;
